...
class Helpers(commands.Cog):
    ...
    @staticmethod
    async def add_day_emojis(msg, days):
        for day in days:
            await msg.add_reaction(Constants.day_emojis[day])

    @staticmethod
    async def add_time_emojis(msg, times):
        for time in times:
    ...
    def find_available_evaluators(evaluation_time, course, guild):
        available_evaluators = []
        for evaluator in guild.members:
            if any(role.name == 'Evaluator' for role in evaluator.roles):
                db_management.c.execute("SELECT * FROM evaluators WHERE id=?", (evaluator.id,))

                # evaluator: (id, name, available, courses)
                # evaluator available: 'm,a,e,ma,me,ae,'
                # courses: 'spj'
                # had to use a different variable name
                person = db_management.c.fetchone()
                availability = person[2].split(',')[evaluation_time[0]]
                courses = person[3]

                if evaluation_time[1] in availability and course in courses:
